...
#include "AXP/xplatform/include/stl/hashtable.h"
...
namespace IDLC
{
    ...
    STATIC Boolean HasObjectType(IN TreeNode<CSymbol> * node);
    ...
    STATIC Void WriteVarToParcel(IN TreeNode<CSymbol> * node)
    {
        ...
        if (varSymbol->mSymbolType == SymbolType_Class) {
            WRITE_STRING_TO_FILE(varId->Length() + 97,
                L"if (AResult.AFAILED(ObjectParcel.WriteObjectToParcel(parcel, %ls)))\r\n", (PCWStr)*varId);
            ...
            if (elementSymbol->mSymbolType == SymbolType_Class) {
                WRITE_STRING_TO_FILE(varId->Length() + 97,
                    L"if (AResult.AFAILED(ObjectParcel.WriteListOfObjectToParcel(parcel, %ls)))\r\n", (PCWStr)*varId);
                WRITE_STRING_TO_FILE(failedMsg);
            }
    ...
    STATIC Void ReadVarFromParcel(IN TreeNode<CSymbol> * node)
    {
        ...
        if (varSymbol->mSymbolType == SymbolType_Class) {
            if (node->GetValue()->mSymbolType == SymbolType_Function)
                ...
                WRITE_STRING_TO_FILE(varId->Length() + 17,
                L"%ls = ", (PCWStr)*varId);

            FWRITE("ObjectParcel.ReadObjectFromParcel(parcel);\r\n");
        }
        else if (varSymbol->mSymbolType == SymbolType_Interface) {
            if (node->GetValue()->mSymbolType == SymbolType_Function)
            ...
            if (elementSymbol->mSymbolType == SymbolType_Class) {
                if (node->GetValue()->mSymbolType == SymbolType_Function)
                    ...
                    WRITE_STRING_TO_FILE(varId->Length() + 17, L"%ls = ", (PCWStr)*varId);

                FWRITE("ObjectParcel.ReadListOfObjectFromParcel(parcel);\r\n");
            }
            else if ((elementSymbol->mSymbolType == SymbolType_String)
                || (elementSymbol->mSymbolType == SymbolType_ByteArray)
                || (elementSymbol->mSymbolType == SymbolType_Int8_NULL)
                ...
                if ((node->GetValue()->mSymbolType == SymbolType_Function)
                    || (node->GetValue()->mSymbolType == SymbolType_Parameter))
                    WRITE_STRING_TO_FILE(varType->Length() + varId->Length() + 7,
    ...
    STATIC Void WriteObjectHolderOnTransact(IN TreeNode<CSymbol> * node)
    {
        ...
        FWRITE("public CParcel OnTransact(CParcel parcel, String uri)\r\n{\r\n");
    ...
    STATIC Void WriteProxyConstructions(IN TreeNode<CSymbol> * node)
    {
        ...
        Foreach(TreeNode<CSymbol>, obj, node->GetChildren()) {
            if (obj->GetValue() && obj->GetValue()->mSymbolType == SymbolType_Construction) {
                ...
                FWRITE("CParcel parcel = new CParcel();\r\n\r\n");
    ...
    STATIC Void WriteStubFile(IN TreeNode<CSymbol> * node)
    {
        ...
        Sp<String> dirPath = String::Create(gRootOutputDir->Length() + szNamespace->Length() + 37,
            L"%ls/java/stub/%ls/", (PCWStr)*gRootOutputDir, (PCWStr)*szNamespace);
        if (dirPath == NULL)
            return;
        ...
        WRITE_STRING_TO_FILE(szNamespace->Length() + 77, L"package IPC.gen.java.stub.%ls;\r\n\r\n", (PCWStr)*szNamespace);
        ...
        FWRITE("public CParcel Transact(CParcel bundle)\r\n");
        FWRITE("{\r\n");
        ...
        FWRITE("private CObjectHolder CreateOrGetObjectHolder("
            "int code, String uri, CParcel bundle, CParcel parcel)\r\n");
        FWRITE("{\r\n");
        FWRITE("if (bundle == null)\r\nreturn null;\r\n\r\n");
    ...
    STATIC Void WriteObjectHolderFile(IN TreeNode<CSymbol> * node)
    {
        ...
        Boolean hasObjectType = HasObjectType(node);
        if (hasObjectType)
            FWRITE("import IPC.java.ObjectParcel;\r\n");
        
        WRITE_STRING_TO_FILE(szNamespace->Length() + fileNameWithoutSuffix->Length() + 77,
            L"import IPC.gen.java.include.%ls.I%ls;\r\n",
    ...
    STATIC Void WriteInterfaceFile(IN TreeNode<CSymbol> * node)
    {
        ...
        Sp<String> dirPath = String::Create(gRootOutputDir->Length() + namespacz->Length() + 37,
            L"%ls/java/include/%ls/", (PCWStr)*gRootOutputDir, (PCWStr)*namespacz);
        if (dirPath == NULL)
            return;
        ...
        WRITE_STRING_TO_FILE(namespacz->Length() + 77, L"package IPC.gen.java.include.%ls;\r\n\r\n", (PCWStr)*namespacz);
    ...
    STATIC Void WriteServiceFile(IN TreeNode<CSymbol> * node)
    {
        ...
        Sp<String> dirPath = String::Create(gRootOutputDir->Length() + namespacz->Length() + 37,
            L"%ls/java/service/%ls/", (PCWStr)*gRootOutputDir, (PCWStr)*namespacz);
        if (dirPath == NULL)
            return;
        ...
        WRITE_STRING_TO_FILE(namespacz->Length() + 77, L"package IPC.gen.java.service.%ls;\r\n\r\n", (PCWStr)*namespacz);
        WriteJvmImportFile(node, E_INTERFACE);
        ...
        WRITE_STRING_TO_FILE(className->Length() * 2 + 77, L"public class %ls implements I%ls\r\n", (PCWStr)*className, (PCWStr)*className);
        FWRITE("{\r\n");
    ...
    STATIC Void WriteMembersToParcel(IN TreeNode<CSymbol> * node)
    {
        ...
        FWRITE("public int WriteToParcel(CParcel parcel)\r\n");
        FWRITE("{\r\n");
        FWRITE("if (parcel == null)\r\nreturn AResult.AE_FAIL;\r\n\r\n");
    ...
    STATIC Void ReadMembersFromParcel(IN TreeNode<CSymbol> * node)
    {
        ...
        FWRITE("public int ReadFromParcel(CParcel parcel)\r\n");
        FWRITE("{\r\n");
        FWRITE("if (parcel == null)\r\nreturn AResult.AE_FAIL;\r\n\r\n");
    ...
    STATIC Void WriteClassNameInsertMappingTable(IN TreeNode<CSymbol> * node)
    {
        ...
        WRITE_STRING_TO_FILE(varName->Length() + description->Length() + scopeOfClass->Length() + 97,
            L"STATIC AXP::Boolean _%ls_ = ObjectParcel.InsertMappingTable(L\"%ls\", %ls::Create);\r\n",
    ...
    STATIC Void WriteModelsFile(IN TreeNode<CSymbol> * node)
    {
        ...
        Sp<String> dirPath = String::Create(gRootOutputDir->Length() + namespacz->Length() + 77,
            L"%ls/java/models/%ls/", (PCWStr)*gRootOutputDir, (PCWStr)*namespacz);
        if (dirPath == NULL)
            return;
        ...
        WRITE_STRING_TO_FILE(namespacz->Length() + 77, L"package IPC.gen.java.models.%ls;\r\n\r\n", (PCWStr)*namespacz);
        Int32 writen = 0;
        Foreach(TreeNode<CSymbol>, obj, node->GetChildren()) {
            if (obj->GetValue()->mSymbolType == SymbolType_Member) {
    ...
    STATIC Void WriteProxyFile(IN TreeNode<CSymbol> * node)
    {
        ...
        Sp<String> dirPath = String::Create(gRootOutputDir->Length() + szNamespace->Length() + 77,
            L"%ls/java/proxy/%ls/", (PCWStr)*gRootOutputDir, (PCWStr)*szNamespace);
        if (dirPath == NULL)
            return;
        ...
        WRITE_STRING_TO_FILE(szNamespace->Length() + 77, L"package IPC.gen.java.proxy.%ls;\r\n\r\n", (PCWStr)*szNamespace);
