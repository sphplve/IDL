...
namespace IDLC
{
    ...
    STATIC Void ReadVarFromParcel(IN TreeNode<CSymbol> * node)
    {
        ...
        if (varSymbol->mSymbolType == SymbolType_Class) {
            WRITE_STRING_TO_FILE(varId->Length() + 97,
            L"if (AResult.AFAILED(ObjectParcel.ReadObjectFromParcel(parcel, ref %ls)))\r\n", (PCWStr)*varId);
            WRITE_STRING_TO_FILE(failedMsg);
            FWRITE("\r\n");
        }
        else if (varSymbol->mSymbolType == SymbolType_Interface) {
            ...
            if (elementSymbol->mSymbolType == SymbolType_Class) {
                WRITE_STRING_TO_FILE(varId->Length() + 97,
                L"if (AResult.AFAILED(ObjectParcel.ReadListOfObjectFromParcel(parcel, ref %ls)))\r\n", (PCWStr)*varId);
                WRITE_STRING_TO_FILE(failedMsg);
                FWRITE("\r\n");
            }
            else if ((elementSymbol->mSymbolType == SymbolType_String)
                ...
                WRITE_STRING_TO_FILE(failedMsg);
                FWRITE("\r\n");
                FWRITE("Int32 length = System.BitConverter.ToInt32(lengthArray, 0);\r\n");
    ...
    STATIC Void WriteMembersToParcel(IN TreeNode<CSymbol> * node)
    {
        ...
        if (isInheritInterface)
            FWRITE("public virtual Int32 WriteToParcel(CParcel parcel)\r\n");
        else
            FWRITE("public override Int32 WriteToParcel(CParcel parcel)\r\n");
    ...
    STATIC Void ReadMembersFromParcel(IN TreeNode<CSymbol> * node)
    {
        ...
        if (isInheritInterface)
            FWRITE("public virtual Int32 ReadFromParcel(CParcel parcel)\r\n");
        else
            FWRITE("public override Int32 ReadFromParcel(CParcel parcel)\r\n");

        FWRITE("{\r\n");
        FWRITE("if (parcel == null)\r\nreturn AResult.AE_FAIL;\r\n\r\n");
        FWRITE("String className = null;\r\n");
        FWRITE("if (AResult.AFAILED(parcel.ReadString(ref className)))\r\nreturn AResult.AE_FAIL;\r\n\r\n");
        if (!isInheritInterface)
    ...
    STATIC Void WriteModelsFile(IN TreeNode<CSymbol> * node, IN CONST Sp<String> & fileNameWithoutSuffix)
    {
        ...
        Sp<String> dirPath = String::Create(gRootOutputDir->Length() + namespacz->Length() + 38, L"%ls/csharp/%ls/",
