...
#define MKDIR(a) mkdir((a), 0777)
    ...
    Sp<String> GetConsOrFuncId(IN TreeNode<CSymbol> * node)
    {
        ...
        return String::Create(className->Length() + funcName->Length() + 7, L"%ls_%ls", (PCWStr)*className, (PCWStr)*funcName);
    }
    ...
    Sp<String> GetDefConsId(IN TreeNode<CSymbol> * node)
    {
        ...
        return String::Create(funcName->Length() * 2 + 7, L"%ls_%ls", (PCWStr)*funcName, (PCWStr)*funcName);
    }
    ...
    Sp<String> GetDESCRIPTORId(IN TreeNode<CSymbol> * node)
    {
        ...
        return String::Create(className->Length() + 37, L"%ls_DESCRIPTOR", (PCWStr)*className);
    }
    ...
    Sp<String> GetNameSpace(IN TreeNode<CSymbol> * node)
    {
        ...
        else if ((node->GetValue()->mSymbolType == SymbolType_Begin) || (node->GetValue()->mSymbolType == SymbolType_FileBegin)) {
            Foreach(TreeNode<CSymbol>, obj, node->GetChildren()) {
                if (obj->GetValue() == NULL)
                    return NULL;
