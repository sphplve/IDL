...
#include <fcntl.h>
#include <dirent.h>  
#include "AXP/xplatform/include/astring.h"
            ...
            if (fpid < 0) {
                ...
                if (gIDLFlag & IDL_LANG_JAVASCRIPT) {
                    ...
                    while (true) {
                        ...
                        L", \"IPC/gen/javascript/stub/%ls/%lsStub\"", (PCWStr)*namespacz, (PCWStr)*sFileName);
                        if (importFile == NULL)
                            return -1;
...
STATIC Void WriteCppMapping()
{
    ...
    Foreach(CMappingInfo, obj, mappingList) {
        WRITE_STRING_TO_FILE(obj->mDescription->Length() + obj->mScope->Length() + 87,
            L"if (!ObjectParcel::InsertMappingTable(L\"%ls\", %ls::Create))\r\n", (PCWStr)*obj->mDescription, (PCWStr)*obj->mScope);
        FWRITE("return FALSE;\r\n\r\n");
    }
...
STATIC Void WriteCsMapping()
{
    ...
    Sp<String> dirPath = String::Create(gRootOutputDir->Length() + 24, L"%ls/csharp/", (PCWStr)*gRootOutputDir);
    if (dirPath == NULL)
        return;
    ...
    Foreach(CMappingInfo, obj, mappingList) {
        WRITE_STRING_TO_FILE(obj->mDescription->Length() + obj->mScope->Length() + 87,
            L"if (!ObjectParcel.InsertMappingTable(\"%ls\", %ls.Create))\r\n", (PCWStr)*obj->mDescription, (PCWStr)*obj->mScope);
        FWRITE("return false;\r\n\r\n");
    }
...
STATIC Void WriteJavascriptStubClassRegister()
{
    ...
    Sp<String> dirPath = String::Create(gRootOutputDir->Length() + 77, L"%ls/javascript/", (PCWStr)*gRootOutputDir);
    if (dirPath == NULL)
        return;
...
int main(int argc, char* argv[])
{
    ...
    int ch;
    opterr = 0;
    while ((ch = getopt(argc, argv, "j:c:m:i:o:d:hgp")) != -1) {
        switch (ch) {
        case 'j':
