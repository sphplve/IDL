...
namespace IDLC
{
    ...
    STATIC Void ReadVarFromParcel(IN TreeNode<CSymbol> * node)
    {
        ...
        if (node->GetValue()->mSymbolType == SymbolType_Parameter) {
            ...
            varId = String::Create(varId->Length() + 17, L"var %ls", (PCWStr)*varId);            
        }
        else if (node->GetValue()->mSymbolType == SymbolType_Member) {
            if ((node->GetChildren()->Get(1) == NULL) || (node->GetChildren()->Get(1)->GetValue() == NULL))
                return;
        ...
        if (varSymbol->mSymbolType == SymbolType_Class) {
            FWRITE("var hasValue = parcel.readBooelan();\r\nif (hasValue) {\r\n"
            ...
            if (node->GetValue()->mSymbolType == SymbolType_Function)
                FWRITE("ret.readFromParcel(parcel);\r\n}\r\n");
            else if (node->GetValue()->mSymbolType == SymbolType_Parameter)
                WRITE_STRING_TO_FILE(node->GetChildren()->Get(1)->GetValue()->mContent->Length() + 97,
                L"%ls.readFromParcel(parcel);\r\n}\r\n", (PCWStr)*node->GetChildren()->Get(1)->GetValue()->mContent);
            else
                WRITE_STRING_TO_FILE(varId->Length() + 97, L"%ls.readFromParcel(parcel);\r\n}\r\n", (PCWStr)*varId);
            
            WRITE_STRING_TO_FILE(varId->Length() + 30, L"else\r\n%ls = null;\r\n\r\n", (PCWStr)*varId);
        }
        else if (varSymbol->mSymbolType == SymbolType_Interface)
            ...
            *pname += 0x20;
            ...
            if (elementSymbol->mSymbolType == SymbolType_Class) {
                ...
                "var clazz = parcel.get(parcel.getPosition());\r\n if (!clazz)\r\nreturn;\r\n\r\n"
            ...
            if (obj->GetValue()->mSymbolType == SymbolType_FileBegin) {
                ...
                if (mode & IDL_MODE_IPC) {
                    if (modeFlag & IDL_MODE_IPC)
                        ...
                        L"\"IPC/gen/javascript/proxy/%ls/%ls\"", (PCWStr)*tmp, (PCWStr)*fileNameWithoutSuffix);
                    else
                        WRITE_STRING_TO_FILE(fileNameWithoutSuffix->Length() + tmp->Length() + 77,
                        L"\"IPC/gen/javascript/models/%ls/%ls\"", (PCWStr)*tmp, (PCWStr)*fileNameWithoutSuffix);
                }
    ...
    STATIC Void WriteModelsFile(IN TreeNode<CSymbol> * node, IN CONST Sp<String> & fileNameWithoutSuffix)
    {
        ...
        Sp<String> dirPath = String::Create(gRootOutputDir->Length() + namespacz->Length() + 77,
            L"%ls/javascript/models/%ls/", (PCWStr)*gRootOutputDir, (PCWStr)*namespacz);
        if (dirPath == NULL)
            return;
    ...
    STATIC Void WriteInterfaceFile(IN TreeNode<CSymbol> * node, IN CONST Sp<String> & fileNameWithoutSuffix)
    {
        ...
        Sp<String> dirPath = String::Create(gRootOutputDir->Length() + szNamespace->Length() + 37,
            L"%ls/javascript/include/%ls/", (PCWStr)*gRootOutputDir, (PCWStr)*szNamespace);
        if (dirPath == NULL)
            return;
...
puts("aaa");
    ...
    STATIC Void WriteServiceFile(IN TreeNode<CSymbol> * node, IN CONST Sp<String> & fileNameWithoutSuffix)
    {
        ...
        Sp<String> dirPath = String::Create(gRootOutputDir->Length() + szNamespace->Length() + 37,
            L"%ls/javascript/service/%ls/", (PCWStr)*gRootOutputDir, (PCWStr)*szNamespace);
        if (dirPath == NULL)
            return;

        GenerateDirPath(dirPath);
        Sp<String> fullFileName = String::Create(dirPath->Length() + szNamespace->Length() + fileNameWithoutSuffix->Length() + 77,
            L"%ls/%ls.js", (PCWStr)*dirPath, (PCWStr)*szNamespace, (PCWStr)*fileNameWithoutSuffix);
        if (fullFileName == NULL)
            return;
        ...
        L"\"IPC/gen/javascript/include/%ls/I%ls\"", (PCWStr)*szNamespace, (PCWStr)*fileNameWithoutSuffix);
        FWRITE("], function () {\r\n");
    ...
    STATIC Void WriteObjectHolderFile(IN TreeNode<CSymbol> * node, IN CONST Sp<String> & fileNameWithoutSuffix)
    {
        ...
        Sp<String> dirPath = String::Create(gRootOutputDir->Length() + szNamespace->Length() + 37,
            L"%ls/javascript/stub/%ls/", (PCWStr)*gRootOutputDir, (PCWStr)*szNamespace);
        if (dirPath == NULL)
            return;
        ...
        WRITE_STRING_TO_FILE(szNamespace->Length() + fileNameWithoutSuffix->Length() + 88,
            L"\"IPC/gen/javascript/service/%ls/%ls\"", (PCWStr)*szNamespace, (PCWStr)*fileNameWithoutSuffix);
        FWRITE("], function () {\r\n");
    ...
    STATIC Void WriteProxyFile(IN TreeNode<CSymbol> * node, IN CONST Sp<String> & fileNameWithoutSuffix)
    {
        ...
        Sp<String> dirPath = String::Create(gRootOutputDir->Length() + szNamespace->Length() + 37,
            L"%ls/javascript/proxy/%ls/", (PCWStr)*gRootOutputDir, (PCWStr)*szNamespace);
        if (dirPath == NULL)
            return;
    ...
    STATIC Void WriteStubFile(IN TreeNode<CSymbol> * node, IN CONST Sp<String> & fileNameWithoutSuffix)
    {
        ...
        L"\"IPC/gen/javascript/stub/%ls/%ls\"", (PCWStr)*szNamespace, (PCWStr)*fileNameObjectHolder);
        FWRITE("], function () {\r\n");
